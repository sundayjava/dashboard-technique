...
// PATCH - Update bank deposit
export async function PATCH(request: Request, { params }: RouteParams) {
  try {
    const { id } = await params;
    const body = await request.json();
    const { amount, status, adminNotes, referenceNumber } = body;
    ...
    try {
      if (status === 'COMPLETED') {
        ...
        if (userEmail) {
          await sendEmail({
            to: userEmail,
            subject: `Bank deposit approved - ${deposit.currency} ${deposit.amount}`,
            html: `<div style="font-family: Arial, sans-serif; max-width:600px;margin:0 auto;">\
              <h2>Bank Deposit Approved</h2>\
              <p>Hello ${userName || 'user'},</p>\
              <p>Your bank deposit of <strong>${deposit.currency} ${deposit.amount}</strong> has been approved and credited to your account.</p>\
      ...
      console.error('Error sending bank deposit email:', emailErr);
    }
    ...
    return NextResponse.json(
      { error: 'Failed to update bank deposit' },
      { status: 500 }
    );
  }
}

// DELETE - Delete bank deposit
export async function DELETE(request: Request, { params }: RouteParams) {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to delete bank deposit' },
      { status: 500 }
    );
  }
}
