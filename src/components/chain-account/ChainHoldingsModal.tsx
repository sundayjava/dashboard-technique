...
interface ChainHoldingsModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function ChainHoldingsModal({
  isOpen,
  onClose,
  chainAccountId,
  accountBalance,
  ...
  const fetchHoldings = async () => {
    try {
      ...
      const response = await axios.get(
        `/api/chain-account/holdings?chainAccountId=${chainAccountId}`,
        authHeaders()
      );
  ...
  const handleCreateHolding = async () => {
    ...
    if (depositAmount > accountBalance) {
      alert('Insufficient Chain Account balance');
      return;
    }
    ...
    try {
      setCreating(true);
      await axios.post(
        '/api/chain-account/holdings',
        { chainAccountId, tokenId: selectedToken.id, amount: depositAmount },
        authHeaders()
      );

      alert(`Holding request submitted for ${depositAmount} ${accountCurrency} in ${selectedToken.name}.\n\nAll members have been notified. An admin will review and approve this request.`);
      setAmount('');
  ...
  const handleWithdraw = async (holdingId: string) => {
    if (!confirm('Are you sure you want to withdraw this holding back to the Chain Account balance?')) return;

    try {
      const response = await axios.patch(
        '/api/chain-account/holdings',
        { chainAccountId, holdingId },
        authHeaders()
      );

      alert(`Successfully withdrawn ${response.data.amount.toFixed(2)} ${accountCurrency} to the Chain Account balance!`);
      fetchHoldings();
      onSuccess();
    } catch (error: any) {
      ...
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
            ...
            <div className="space-y-6">
              <div className="bg-linear-to-r from-blue-50 to-purple-50 rounded-lg md:p-6 p-2">
                <h3 className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-2">
                  ...
                  How Chain Account Holdings Work
                </h3>
                <div className="space-y-3 text-sm text-gray-700">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">
                        Earn daily interest on the Chain Account's holdings based on the annual percentage yield (APY)
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">
                        Withdraw the holding + interest back to the Chain Account balance at any time
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <p className="text-sm text-yellow-800">
                  <strong>Note:</strong> Any member can request a holding. Funds are deducted from the shared
                  balance immediately, and the request awaits admin approval before becoming active. All other
                  members are notified by email when a holding is requested.
                </p>
              </div>
          ...
          {step === 'deposit' && selectedToken && (
            <div className="space-y-4">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-gray-900">Request Holding</h3>
                <button
                  onClick={() => {
                    setStep('select');
                    setSelectedToken(null);
              ...
              {amount && parseFloat(amount) > 0 && selectedToken && (
                <div className="bg-linear-to-br from-blue-50 to-indigo-50 rounded-xl md:p-5 p-2.5 border-2 border-blue-200 shadow-sm">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <p className="text-sm font-semibold text-gray-800">Holding Preview</p>
                  </div>
                  <div className="md:space-y-3 space-y-1.5">
                    <div className="bg-white/70 backdrop-blur-sm rounded-lg md:p-3 p-1.5">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-xs text-gray-600">Will receive</span>
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-2">
                      <p className="text-xs text-yellow-800 flex items-start gap-1">
                        ...
                        <span>This request will be deducted from the shared balance now and needs admin approval to become active. Other members will be notified by email.</span>
                      </p>
                    </div>
                  </div>
                </div>
              ...
              >
                {creating ? 'Submitting Request...' : 'Submit for Approval'}
              </button>
            </div>
          )}
          ...
          {step === 'my-holdings' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-gray-900">Chain Account Holdings</h3>
                <button
                  onClick={() => setStep('intro')}
                  className="text-sm text-blue-600 hover:text-blue-700"
                >
                            ...
                            <div key={holding.id} className="border-2 border-yellow-300 bg-yellow-50 rounded-lg p-4">
                              ...
                              <div className="mt-3 bg-yellow-100 border border-yellow-300 rounded p-2">
                                <p className="text-xs text-yellow-800">
                                  ⏳ Awaiting admin approval. All members have been notified.
                                </p>
                              </div>
                            </div>
                          ))}
